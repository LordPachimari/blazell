...
function Sortable<TData extends { id: UniqueIdentifier }>({
	...
	const currentStrategy = strategy ?? config.strategy;
...
interface SortableItemProps extends SlotProps {
	...
	/**
	 * Specifies class names for elements that should not trigger dragging
...
const SortableItem = React.forwardRef<HTMLDivElement, SortableItemProps>(
	({ value, asTrigger, asChild, className, disableDragOn, ...props }, ref) => {
		const {
			attributes,
			listeners,
			setNodeRef,
		...
		const handleMouseDown = (e: React.MouseEvent) => {
			if (disableDragOn && (e.target as HTMLElement).closest(disableDragOn)) {
				e.stopPropagation();
			} else if (asTrigger && listeners?.onMouseDown) {
		...
		return (
			<SortableItemContext.Provider value={context}>
				<Comp
					...
					className={cn(
						"data-[state=dragging]:cursor-grabbing relative focus-visible:outline-none",
						className,
					)}
					ref={composeRefs(ref, setNodeRef as React.Ref<HTMLDivElement>)}
					style={style}
					{...(asTrigger ? attributes : {})}
					{...(asTrigger ? { ...listeners, onMouseDown: handleMouseDown } : {})}
					{...props}
				/>
...
const SortableDragHandle = React.forwardRef<
	HTMLButtonElement,
	SortableDragHandleProps & { className?: string }
>(({ className, ...props }, ref) => {
