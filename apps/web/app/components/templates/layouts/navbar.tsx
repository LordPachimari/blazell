...
function Navbar(props: { children: React.ReactNode }) {
	...
	return (
		<header
			{...props}
			className={cn(
				"group hidden backdrop-blur-md fixed px-20 inset-x-0 top-0 z-30 left-1/2 transform -translate-x-1/2  md:flex items-center justify-between border border-mauve-7 py-2 transition-all duration-300 hover:border bg-component",
				noHeaderPaths(location.pathname) && "hidden md:hidden",
				requestInfo.userPrefs.sidebarState &&
